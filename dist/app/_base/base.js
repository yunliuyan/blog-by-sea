define(function(require, exports, module){
    ...
    exports.getCategory = function (details,topicList,cb) {
        if(0 == details.length){
            cb([]);
        }else {
            var cate = 0;
            ...
            topicList.forEach(function (item) {
                if(0 == item['pid']){
                    topicList.forEach(function (parent) {
                      //console.log(parent);
                        if(parent['pid'] == item['id']){
                            //console.log(item);
                            parent['name'] = item['name'] + ">>" + parent['name'];
                            if(parent['id'] == details[0]['category_id']){
                                cate = parent['name'];
                                return cb(cate);
                            }
                        }else {
                            if(item['id'] == details[0]['category_id']){
                                cate = item['name'];
                                return cb(cate);
                            }
                        }
